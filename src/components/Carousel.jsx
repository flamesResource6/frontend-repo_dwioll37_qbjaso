    ...
    <section className="bg-white py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-4 text-center text-zinc-800">
          <p className="text-sm uppercase tracking-wider text-zinc-500">Featured</p>
          <p className="text-xl font-medium mt-1">{items[index].title}</p>
        </div>
        <div className="mt-3 flex justify-center gap-2">
          {items.map((_, i) => (
            <button key={i} onClick={() => setIndex(i)} className={`h-2 w-2 rounded-full ${i === index ? 'bg-zinc-900' : 'bg-zinc-300'}`} aria-label={`Go to slide ${i+1}`} />
          ))}
        </div>
      </div>
    </section>
